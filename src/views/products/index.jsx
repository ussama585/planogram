...
export default function ProductsPage() {
  ...
  const categoryOptions = useMemo(() => {
    if (Array.isArray(categoryOptionsData)) return categoryOptionsData;
    if (Array.isArray(categoryOptionsData?.results)) return categoryOptionsData.results;
    if (Array.isArray(categoryOptionsData?.data)) return categoryOptionsData.data;
    return [];
  }, [categoryOptionsData]);

  const brandOptions = useMemo(() => {
    if (Array.isArray(brandOptionsData)) return brandOptionsData;
    if (Array.isArray(brandOptionsData?.results)) return brandOptionsData.results;
    if (Array.isArray(brandOptionsData?.data)) return brandOptionsData.data;
    return [];
  }, [brandOptionsData]);
  ...
  const updateProductMutation = useMutation({
    mutationFn: async (values) => {
      ...
      const response = await api.patch(`/api/inventory/product-detail/${selectedProduct?.id}`, payload);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['product-list'] });
  ...
  return (
    <>
      <MainCard title="Products" secondary={<Button variant="contained" onClick={handleOpenCreate}>Add Product</Button>}>
        <Stack spacing={2}>
          ...
          <TableContainer component={Paper} variant="outlined">
            {isLoading ? (
              ...
              <Stack alignItems="center" justifyContent="center" sx={{ py: 6 }}>
                <Typography color="error">Failed to load products: {error?.message || 'Unknown error'}</Typography>
              </Stack>
            ) : (
              <Table size="small">
                <TableHead>
      ...
      <Dialog open={deleteOpen} onClose={handleDeleteClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete <strong>{selectedProduct?.name || 'this product'}</strong>?
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={handleDeleteClose}>No</Button>
          <Button variant="contained" color="error" onClick={handleDeleteConfirm} disabled={deleteProductMutation.isPending}>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
              ...
              <form onSubmit={handleSubmit}>
                <Stack spacing={2} sx={{ mt: 1 }}>
                  <FormControl fullWidth error={Boolean(touched.name && errors.name)}>
                    ...
                    <OutlinedInput id="product-name" name="name" label="Name" value={values.name} onChange={handleChange} onBlur={handleBlur} />
                    ...
                    >
                      {categoryOptions.map((category) => (
                        <MenuItem key={category?.id} value={category?.id}>
                          {category?.name || category?.category_name || category?.title || category?.id}
                        </MenuItem>
                      ))}
                    </Select>
                    ...
                    >
                      {brandOptions.map((brand) => (
                        <MenuItem key={brand?.id} value={brand?.id}>
                          {brand?.name || brand?.brand_name || brand?.title || brand?.id}
                        </MenuItem>
                      ))}
                    </Select>
                  ...
                  <FormControl fullWidth error={Boolean(touched.sku && errors.sku)}>
                    ...
                    <OutlinedInput id="product-sku" name="sku" label="SKU" value={values.sku} onChange={handleChange} onBlur={handleBlur} />
                  ...
                  <FormControl fullWidth error={Boolean(touched.price && errors.price)}>
                    ...
                    <OutlinedInput id="product-price" name="price" label="Price" type="number" value={values.price} onChange={handleChange} onBlur={handleBlur} />
                  ...
                  <FormControl fullWidth error={Boolean(touched.stock && errors.stock)}>
                    ...
                    <OutlinedInput id="product-stock" name="stock" label="Stock" type="number" value={values.stock} onChange={handleChange} onBlur={handleBlur} />
                  ...
                  <FormControl fullWidth>
                    ...
                    <OutlinedInput id="product-description" name="description" label="Description" multiline minRows={3} value={values.description} onChange={handleChange} onBlur={handleBlur} />
                  </FormControl>
                </Stack>
              </form>
            )}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          >
            {createProductMutation.isPending || updateProductMutation.isPending ? 'Saving...' : selectedProduct ? 'Update Product' : 'Save Product'}
          </Button>
        </DialogActions>
      </Dialog>
    </>
